...
class TestAPIIntegration:
    ...
    def test_cors_headers(self, client: TestClient):
        ...
        response = client.get("/api/v1/health/live", headers={"Origin": "http://localhost:3000"})
        
        # Check that CORS headers are present
        assert "access-control-allow-origin" in response.headers
        assert response.headers["access-control-allow-origin"] == "*"

    def test_authentication_flow(self, client: TestClient, valid_login_data, auth_headers):
    ...
    def test_api_response_format_consistency(self, client: TestClient):
        ...
        for endpoint, method in endpoints:
            ...
            assert response.status_code in [200, 401, 403, 404, 422]  # Valid status codes
    ...
    def test_api_key_management_flow(self, client: TestClient, valid_login_data, 
    ...
    def test_token_expiration_handling(self, client: TestClient, valid_login_data, auth_headers):
        ...
        malformed_response = client.get("/api/v1/auth/profile", headers=malformed_headers)
        assert malformed_response.status_code == 401

    def test_concurrent_api_key_creation(self, client: TestClient, valid_login_data, auth_headers):
        ...
        for name in key_names:
            ...
            response = client.post("/api/v1/auth/api-keys", json=key_request, headers=headers)
            assert response.status_code == 200
